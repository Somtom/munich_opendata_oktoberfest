---
title: Short Oktoberfest Analysis
...
### Did the ZLF bring more visitor to the Oktoberfest (remove outlier 2001)

```{r}
dt %>% 
  ggplot() +
  geom_col(aes(x = year, y = visitors_total, fill = zlf))

dt %>% 
  ggplot(aes(x = zlf, y = visitors_total)) +
  geom_violin(aes(x = zlf, y = visitors_total)) +
...
dt %>% 
  ggplot() +
  geom_density(aes(x = beer_cons/visitors_total, fill = zlf), alpha = 0.4) 

dt %>% 
  ggplot() +
  geom_boxplot(aes(x = zlf, y = beer_cons/visitors_total, fill = zlf), alpha = 0.4) 
