...
Again, we start with a colored plot of the hendl consumption development and two boxplots:
...
The data distribution looks again pretty similar for years with and without ZLF.
The ZLF boxplot is just litte bit more widespread than the other. Nevertheless, we will 
performe our tests as before:
...
distributed. That means we definately need a non parametric test to compare the group means.
...
The Wilcoxon test confirms our impression that ***here is no statistical difference between the mean
hendl consumption of both groups**

## Mean daily visitor count before and after 9/11



Finally we will look at the hypothisis that *the mean daily visitor count decreased to a lower level
...
The boxplot seems to support our hypothisis. Let's see what the tests suggest:
...
```{r}
res <- t.test(visitors_day ~ ifelse(year <= 2000, "After 9/11", "Before 9/11"), data = dt, conf.int = T)
res$estimate
```

The t-test confirms our hypothisis. **The estimated decrease in daily visitors after 9/11 is 
`r as.integer(res$estimate[1] - res$estimate[2])` visitors**. 
The 95% confidence intervall for the difference is `r as.integer(res$conf.int[1])` to
`r as.integer(res$conf.int[2])` visitors.
