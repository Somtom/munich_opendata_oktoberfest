...
will look into a public availabe dataset and try to gain some insights about the Oktoberfest.   
In the first part we will load and describe the data. Furthermore we will analyse the price and
consumption of beer and hendl (chicken) over the years.  

In the second part we will have a closer look at the Bavarian Central Agricultural Festival - also 
known as "ZLF" - and its influence on beer- and hendl consumption, as well as visitor count. 
...
The resource-id (`e0f664cf-6dd9-4743-bd2b-81a8b18bd1d2`), which we will use to get our data via the API,
...
We will change this for all English speaking readers. By doing this, we will also perform some
...
well as total visitors, mean daily visitors and the duration of the Oktoberfest in that year.
...
Now we will start to get our hands dirty and work with the Oktoberfest data set.
...
Since there is no missing data in our dataset we can start our analysis.

***

# Data analysis

In our data analysis we will look at the following topics   
...
We will use visualizations and simple modeling techniques to describe the data. Further we are
going to perform statistical tests to compare means in groups.
...
was about **`r round(coefficients(fit_dm)[2], 2)` Cents per year**.
...
In order to answer this question we will start with a visualization of our data:
...
* Oktoberfest gets more and more popular outside of Munich. People from all over the world go 
there in order to drink a "Maß" beer. For people coming from Australia, Sweden,
...
We see that, that the hendl consumption dropped in 2001 after slowly decreasing from 1991 on.
...
We will cover the questions whether or not the ZLF brought more visitors to the Oktoberfest or 
increased the beer and hendl consumption.


### Did the ZLF bring more visitor to the Oktoberfest?

Since the ZLF is at the same location as the Oktoberfest and a lot of farmers (and of course non-farmers as well) are going there in order to see the exhibition, it could have an influence on
visitor count. We will further investigate this hypothesis by looking at our data. 
...
Munich on July 22nd in the same year and because of the general fear of terror attacks.
Actually my research did not find any reasons for the drop in 1988. Even the weather had been good
around the Oktoberfest in that year.   
...
around a lower level since 2001. We will investigate this hypothesis later.   
...
It looks like both box plots are quiet similar. The median of ZLF years is a little bit higher.
...
test for comparison. 
We will use the Shapiro-Wilk test for that purpose. It tests the null hypothesis that
...
Both box plots look similar again but the ZLF year data is some kind of splitted. 
We now will start with a test for normal distribution and 
...
In the last part of our analysis of the ZLF and its influence we will look at the hendl consumption.
The analysis will be performed the same way as above.   
...
The ZLF boxplot is a little bit more widespread than the other. Nevertheless, we will 
...
This time the test for normal distribution suggests that at least the normal year data are not normally
...
level after the 9/11 terror attacks*.   
...
The boxplots show that the distribution of mean daily visitor count before and after 9/11 varies.
...
There are a lot of other reasons which have not been investigated in this analysis.

# Conclusion

In our quick analysis we showed that...   
...
I also learned that the Munich Open Data API is not that difficult to use.
I am planning to keep on looking into the available datasets and maybe try some more advanced
API Queries.

# Acknowledgement

Finnaly I want to say thank you to the people who supported me with ressources for that analysis.
Great thanks goes to *Frank Börger* and the team from Munich Open Data for answering my questions
on the data and providing additional ressources.
...
* Fragen, ob hendl konsum auch nur noch innerhalb der Zelte
